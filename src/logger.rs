...
//Boxed logger setup
pub fn setup_logger(level: LevelFilter, app_only: bool) {
    INIT.call_once(|| {
        if app_only {
            set_boxed_logger(Box::new(VemeetLogger {}))
...
//Logs Vemeet specific messages only
pub struct VemeetLogger {}
...
impl log::Log for VemeetLogger {
    fn enabled(&self, metadata: &Metadata) -> bool {
        metadata.level() <= log::max_level() && metadata.target().starts_with("core::")
    }
    #[cfg(not(test))]
